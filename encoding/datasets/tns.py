...
class TNSCUIDataset(BaseDataset):
    BASE_DIR = 'TNSCUIChallenge2020'
    NUM_CLASS = 2
    def __init__(self, root=os.path.expanduser('~/.encoding/data'), split='train',
                 mode=None, transform=None, target_transform=None, **kwargs):
        super(TNSCUIDataset, self).__init__(
            root, split, mode, transform, target_transform, **kwargs)
        # assert exists and prepare dataset automatically
        root = os.path.join(root, self.BASE_DIR)
        assert os.path.exists(root), "Please setup the dataset using" + \
            "encoding/scripts/prepare_tns.py"
        self.images, self.masks = _get_tnscui_pairs(root, split)
        if split != 'test':
...
def _get_tnscui_pairs(folder, split='train'):
